...
# 列出 excel
@parameter_extraction.route('/list_excel', methods=['GET'])
def list_excel():
    try:
        minio_client = get_minio_client()
        bucket_name = 'extracted-data'
...
# 下载 excel
@parameter_extraction.route('/download_excel', methods=['POST'])
def download_excel():
    data = request.get_json()
...
# 灰度关联分析
@parameter_extraction.route('/gray_correlation_analysis', methods=['POST'])
...
# 生成对应图片
@parameter_extraction.route('/generate_image', methods=['POST'])
def generate_image():
    data = request.get_json()
